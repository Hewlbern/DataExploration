...
CARBOX_COLOUR = (255, 0, 0)
CAR_CENTER_COLOUR = (196, 24, 75)
CAR_COLOURS = [(0, 0, 255)]
MASK_EXIT_COLOR = (66, 183, 42)
...
class ContourDetection(PipelineProcessor):
    ...
    def detect_vehicles(self, fg_mask, context):
        ...
        for (i, contour) in enumerate(contours):
            ...
            centroid = t_d_functions.get_centroid(x, y, w, h)
    ...
    def __call__(self, context):
        ...
        if self.save_image:
            t_d_functions.save_frame(fg_mask, self.image_dir +
...
class Visualizer(PipelineProcessor):
    ...
    def draw_boxes(self, img, pathes, exit_masks=[]):
        for (i, match) in enumerate(pathes):
            ...
            cv2.rectangle(img, (x, y), (x + w - 1, y + h - 1),
                          CARBOX_COLOUR, 1)
            cv2.circle(img, centroid, 2, CAR_CENTER_COLOUR, -1)
    ...
    def __call__(self, context):
        ...
        t_d_functions.save_frame(frame, self.image_dir +
